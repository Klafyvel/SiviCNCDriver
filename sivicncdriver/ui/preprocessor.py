    ...
    def get_minimize_bounding_box(self):
        ...
        for t in parse(self.gcode):
            ...
            if 'Z' in t['args']:
                z = t['args']['Z']
            if (z > 0
                    or t['value'] not in (1, 2, 3)
                    or not ('X' in t['args'] or 'Y' in t['args'])):
            ...
            if 'X' in t['args']:
                ...
                if x < min_x:
                    logger.debug(
                        "min_x becomes {x} from {t}".format(**locals()))
                min_x = min(min_x, x)
            if 'Y' in t['args']:
                y = t['args']['Y']
                if not set_y:
                ...
                if y < min_y:
                    logger.debug(
                        "min_y becomes {y} from {t}".format(**locals()))
                min_y = min(min_y, y)
            ...
            if t['value'] in (2, 3):
                ...
                for xc, yc in segments:
                    ...
                    if xc < min_x:
                        logger.debug(
                            "min_x becomes {xc} from {t}".format(**locals()))
                    if yc < min_y:
                        logger.debug(
                            "min_y becomes {yc} from {t}".format(**locals()))
                    min_x = min(min_x, xc)
                    min_y = min(min_y, yc)
            x_o, y_o = x, y
        logger.debug(
            "Minimizing by setting origin to {},{}".format(min_x, min_y))
        return min_x, min_y
